...
**Fifth status update (2026-07-14) — Phase 10 complete.** Maps are immutable AVL trees with native int64/string keys (50 000 `h_insert`s: **422 ms**, vs not finishing within 115 s before), vectors are a first-class type (`to_vec`/`vec_get`/`vec_len`/`vec_set`/`vec_to_list`, O(1) reads), and `memoize` keys integer arguments without serialization. All plan phases are now closed.

**Sixth status update (2026-07-14) — follow-ups closed.** The manual now documents every builtin, operator, and sugar form with verified examples (plus `~` negation implemented, unknown characters rejected with positioned errors, and infinite ranges `[n..]` added along the way). The Phase 6 CAF follow-up is done: globals are stored as memoized thunk cells evaluated in the root environment, so a top-level constant is computed once per session (`expensive + expensive` costs one evaluation), global function closures are built once instead of per reference (`aoc8.m` improved to **1.20 s**), and `x = x + 1` reports `Infinite loop on identifier: x` instead of spinning. The set API is complete (`s_insert` on the persistent tree). The only remaining future work is the compiler stage: strictness analysis and uncurrying.
