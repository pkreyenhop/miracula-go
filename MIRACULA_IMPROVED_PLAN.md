...
2. **Multi-binding frames** — deliberately deferred: switching `Env` to slice-backed frames doubles the size of the (far more common) one-binding frame and requires touching every binding site; revisit together with item 5, where the evaluation loop is rewritten anyway.
